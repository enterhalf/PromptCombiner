<script lang="ts">
  ...
  import { getWorkspaceItems, createPromptFile, loadPromptFile, renamePromptFile, deletePromptFile, copyPromptFile } from '../tauri-api';
  ...
  function handleSelectWorkspace() {
    ...
    input.onchange = async (e) => {
      ...
      if (files && files.length > 0) {
        workspacePath = files[0].webkitRelativePath.split('/')[0];
  ...
  async function handleCreateFile() {
    if (newFileName.trim()) {
      try {
        const filePath = await createPromptFile(workspacePath, newFileName.trim());
        await loadWorkspace();
...
<div class="w-64 bg-gray-900 border-r border-gray-700 flex flex-col h-full">
  ...
  <div class="flex border-b border-gray-700">
    <button
      ...
      class="flex-1 py-2 text-sm font-medium {activeTab === 'files' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-300'}"
    >
      Files
    </button>
    <button
      on:click={() => handleTabClick('workbench')}
      class="flex-1 py-2 text-sm font-medium {activeTab === 'workbench' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400 hover:text-gray-300'}"
    >
      Workbench
    </button>
  </div>
